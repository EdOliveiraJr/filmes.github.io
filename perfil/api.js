const base_url = "https://api.themoviedb.org/3";
const api_key = "api_key=be5484dd089336fe67ac9edc816f7583";
const img_url = "https://image.tmdb.org/t/p/w300";

const main = document.getElementById("main");
const perfil = document.getElementById("perfil");

function logOut() {
  localStorage.setItem("usuarioLogado", "");
}

async function showPerfil() {
  const usuarioAutenticado = JSON.parse(localStorage.getItem("usuarioLogado"));
  const { email, nome, fotoUrl } = usuarioAutenticado;

  const foto =
    fotoUrl ||
    "./img/portrait-of-superhero-dog-wearing-red-cape-jumping-like-a-super-hero-isolated-on-studio-background-transformed.png";
  perfil.innerHTML = `
        <div class="perfil">
        <img src="${foto}" alt="Foto de ${nome}">   
        <p><strong>Email:</strong> ${email}</p>
        <p><strong>Nome:</strong> ${nome}</p>  
        </div>
    `;
}

async function showFavorites() {
  const usuarioAutenticado = JSON.parse(localStorage.getItem("usuarioLogado"));

  if (usuarioAutenticado.favoriteMovies.length === 0) {
    main.innerHTML =
      '<div class="info">Você ainda não adicionou nenhum filme aos favoritos.</div>';
    hideLoading();
    return;
  }

  try {
    const movieDetails = [];
    for (const movieId of usuarioAutenticado.favoriteMovies) {
      try {
        const url = `${base_url}/movie/${movieId}?${api_key}`;
        const response = await axios.get(url);
        movieDetails.push(response.data);
      } catch (error) {
        console.error(
          `Erro ao obter detalhes do filme com ID ${movieId}:`,
          error
        );
      }
    }
    const favoriteMovies = movieDetails.map((movie) => {
      const { id, title, poster_path, vote_average, overview } = movie;
      return `
                <div class="movie">
                    <a href="../detalhes/detalhes.html?id=${id}">
                        <img src="${img_url + poster_path}" alt="${title}">
                    </a>    
                    <div class="infos">
                        <div class="movie-info">
                            <h3>${title}</h3>
                            <span class="${getColor(
                              vote_average
                            )}">${vote_average}</span>
                        </div>
                        <div class="overview">
                            <h5>Sinopse</h5>
                            <p>${overview}</p>
                        </div>
                    </div>
                </div>
            `;
    });

    main.innerHTML = favoriteMovies.join("");
  } catch (error) {
    console.error(error);
    main.innerHTML =
      '<div class="info">Ocorreu um erro ao carregar os filmes favoritos.</div>';
  }
}

function removeFromFavorites(movieId) {
  let favoriteMovies = JSON.parse(localStorage.getItem("favoriteMovies")) || [];
  favoriteMovies = favoriteMovies.filter((id) => id !== movieId);
  localStorage.setItem("favoriteMovies", JSON.stringify(favoriteMovies));

  showFavorites();
}

function getColor(vote) {
  if (vote >= 8) {
    return "green";
  } else if (vote >= 5) {
    return "orange";
  } else {
    return "red";
  }
}

showFavorites();
showPerfil();
